package com.cc.notes.model;

import com.notes.cc.notes.R;

public class Homeinfo {

    public  static   String name[]={
            "小璐璐","森林精灵","小蘑菇吖","你是居居吗","江湖人称小仙女","你看见我的猫嘛","小拳拳捶你",
            };

    public  static  String age[]={
            "♀ 18","♀ 19","♀ 18","♀ 18","♀ 20",
            "♀ 21","♀ 20","♀ 20"};

    public  static  String xingzuo[]={
            "金牛座","天蝎座","双子座","白羊座","狮子座","金牛座","狮子座"
            };

    public  static  String hangye[]={
            "IT/程序","动漫/二次元","歌手","舞蹈/教师","幼师/教育","健身/教育","直播/主持"};

    public  static  String jianjie[]={
            "Grcen","JACKwomen","HelloWorld","FOLICNNN","INtent","YYn","LLONMN"};

    public  static  String xingqu[]={
            "跑步/羽毛球","唱歌/跳舞","游戏/电竞","电影/小说","动漫/二次元","CosPlay","历史文化"};


    public  static  String gexingqianming[]={
            "想你，念你，脑海里徘徊你的身影;思你，梦你，梦境中残留你的气息;忆你，恋你，心中充满你的点滴。喜欢你，早已不是秘密;爱你，一直在心底。",
            "不是因为寂寞才想你，是因为寂寞时想的只有你。如果找一个给你打电话的理由！我想你一定会失望的，因为我有那么多个理由，想听吗？等会儿接电话好了。",
            "我是焦渴的旅人，你是一泓清泉;我是远航的船，你是港湾的岸。你是我的希望，也是我的归宿。",
            "我在佛祖面前许了一个心愿，希望化座一颗小树，矗立在你每天经过的路旁。我将爱恋与思念挂满枝头，希望有一天你会与我相恋！",
            "在你的脑海有没有一寄扁舟的恻隐，那是我用前世五百年的祈求换来的等待，有风有雨的时候他不回随意的离开，风平浪静的时候他只会慢慢开来。",
            "失望，有时候也是一种幸福，因为有所期待所以才会失望。因为有爱，才会有期待，所以纵使失望，也是一种幸福，虽然这种幸福有点痛。",
            "找一个陪伴，看看末日风景也好；找一个人来爱，暖暖末日心酸多High！2016正走来，快快说出你的爱，找到你的Mr。Right！"};

    public static Integer img[]= {
            R.drawable.image1, R.drawable.image2, R.drawable.image3, R.drawable.image4, R.drawable.image5, R.drawable.image7, R.drawable.image8};


    public static Integer img2[]={
            R.drawable.image1,R.drawable.image2,R.drawable.image3,R.drawable.image4,R.drawable.image5,R.drawable.image7,R.drawable.image8
    };
}
